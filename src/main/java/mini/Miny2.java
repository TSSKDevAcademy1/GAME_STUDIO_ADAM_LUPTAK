...
@WebServlet("/hello1")
public class Miny2 extends HttpServlet {
	private static final String A_HREF_HTTP_LOCALHOST_8080_GAMES_HELLO_X = "<a href=\"?x=";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.print(" {");
		out.print(" window.location.href=\"http://localhost:8080/games1/main.jsf\";");
		out.print("   }");
		out.print(" </script>");
		...
		if (field.getState() == GameState.SOLVED) {
			...
			try {
				if (userlogin.getUser() != null) {
					...
					scoreRegistration.addScore1(scoree1, "mineSweeper", userlogin.getUser());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}

			// userRepositry.addScore(score, userlogin.getUser(),
			// "mineSweeper");
		...
		if (userlogin.getUser() != null) {
			out.print("<h1>MineSweeper " + userlogin.getUser().getName() + "</h1>");
		} else {
			out.print("<h1>MineSweeper - not logged</h1>");
		}
